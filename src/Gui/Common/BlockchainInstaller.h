...
private:
  ...
  const QDir m_chavezcoinDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfChavezcoinBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _chavezcoin) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo chavezcoinBlockchainInfo() const;
